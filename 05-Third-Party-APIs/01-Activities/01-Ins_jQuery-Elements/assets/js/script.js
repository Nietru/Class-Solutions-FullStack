...
var titleEl = $('<h1>'); // the < > creates.

// titleEl.textContent = "Hello friends";
titleEl.text('Hello friends');

// titleEl.className = 'fancy';
titleEl.attr('class', 'fancy'); //adds a class named "fancy".

// titleEl.classList.add('p-5') - (`p-5` is for padding)
titleEl.addClass('p-5');  //can also add a class this way.

// titleEl.style.border = "rgb(122, 242, 242) 3px solid";
titleEl.css('border', 'rgb(122, 242, 242) 5px solid'); // .css lets us style.
...
for (var i = 0; i < abilities.length; i++) {
  ...
  abilityEl.text(abilities[i]); // makes a new list item element for each element in the abilities array.
